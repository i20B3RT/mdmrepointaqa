...
public class CustomHTMLReportListener implements IReporter {
    ...
    private final String cssFileName = "styles.css";
    ...
    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
        ...
        try {
            writer = createWriter(outdir, reportFileName);
        } catch (IOException e) {
        ...
        writer.close();
        try {
            createCssFile(outdir);
        } catch (IOException e) {
            System.err.println("Unable to create css file");
            e.printStackTrace();
        }
    }

    private void createCssFile(String outdir) throws IOException {
        PrintWriter cssWriter = createWriter(outdir, cssFileName);
        cssWriter.println("table {margin-bottom:10px;border-collapse:collapse;empty-cells:show}");
        cssWriter.println("td,th {border:1px solid #009;padding:.25em .5em}");
        cssWriter.println(".result th {vertical-align:bottom}");
        cssWriter.println(".param th {padding-left:1em;padding-right:1em}");
        cssWriter.println(".param td {padding-left:.5em;padding-right:2em}");
        cssWriter.println(".stripe td,.stripe th {background-color: #E6EBF9}");
        cssWriter.println(".numi, .numi_attn {text-align:right}");
        cssWriter.println(".total td {font-weight:bold}");
        cssWriter.println(".passedodd td {background-color: #0A0}");
        cssWriter.println(".passedeven td {background-color: #3F3}");
        cssWriter.println(".skippedodd td {background-color: #CCC}");
        cssWriter.println(".skippedodd td {background-color: #DDD}");
        cssWriter.println(".failedodd td, .numi_attn {background-color: #F33}");
        cssWriter.println(".failedeven td, .stripe .numi_attn {background-color: #D00}");
        cssWriter.println(".stacktrace {white-space:pre;font-family:monospace}");
        cssWriter.println(".totop {font-size:85%;text-align:center;border-bottom:2px solid #000}");
        cssWriter.println(".leftAl {text-align:left;padding-right:2em}");
        cssWriter.println(".rightAl {text-align:right}");
        cssWriter.flush();
        cssWriter.close();
        ...
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                buff.append("<td><a href='#m"
                        ...
                        + testInstanceName + ")") + "</td>"
                        + "<td class='numi leftAl'>" + firstLine + "<br/></td>"
                        + "<td class='rightAl'>" + formatter.format(calendar.getTime()) + "</td>" + "<td class='numi'>"
    ...
    protected void startHtml(PrintWriter out) {
        out.println("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.1//EN\" \"http://www.w3.org/TR/xhtml11/DTD/xhtml1.dtd\">");
        out.println("<html xmlns='http://www.w3.org/1999/xhtml'>");
        out.println("<head>");
        out.println("<title>TestNG Report</title>");
        out.print("<link rel='stylesheet' href='" + cssFileName + "'>");
        //out.print("<link rel='stylesheet' href='../../src/test/resources/styles/customReporter.css'>");
